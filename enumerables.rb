...
def get_spicy_food_by_cuisine(spicy_foods, cuisine)
  result = spicy_foods.find {|food| food[:cuisine] == cuisine}
end

# Given an array of spicy foods, **return an array of hashes** 
# sorted by heat level from lowest to highest
def sort_by_heat(spicy_foods)
  sorted_arrray = spicy_foods.sort_by do |food1|
    food1[:heat_level]
  end  
end
...
def print_spiciest_foods(spicy_foods)
  spicy_foods.select {|food| food[:heat_level]>5}.map {|food| puts "#{food[:name]} (#{food[:cuisine]}) | Heat Level: " + "🌶" * food[:heat_level]}
end
